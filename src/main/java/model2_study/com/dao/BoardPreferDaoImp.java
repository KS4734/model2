...
public class BoardPreferDaoImp implements BoardPreferDao{
	...
	public BoardPreferDto detailFindbyUserIdAndBoardNo(String userId, int boardNo) throws Exception {
		BoardPreferDto boardPerfer = null;
		pstmt=conn.prepareStatement(detailFindbyUserIdAndBoardNoSql);
		pstmt.setString(1, userId);
		pstmt.setInt(2, boardNo);
		rs=pstmt.executeQuery();
		if(rs.next()) {
			boardPerfer = new BoardPreferDto();
			boardPerfer.setBoard_prefer_no(rs.getInt("board_prefer_no"));
			boardPerfer.setBoard_no(rs.getInt("board_no"));
			boardPerfer.setPrefer(rs.getInt("prefer"));
			boardPerfer.setUser_id(rs.getString("user_id"));
		}
		return boardPerfer;
	}
