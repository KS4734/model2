...
public class ReplyUpdateServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			String imgPath = multiReq.getParameter("imgPath");  // 기존이미지
			...
			replyDto.setBoard_no(Integer.parseInt(replyNo_str));
	...
	protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// fetch 에서만 호출할 수 있는 method (form action으로는 호출할 수 없다.)
		int delete = 0;
		HttpSession session=req.getSession();
		Object loginUser_obj=session.getAttribute("loginUser");
		// delete : -1 로그인 하세요
		// delete : -2 작성자만 삭제 가능
		// delete : -3 db오류 파라미터 오류
		String replyNo_str = req.getParameter("replyNo");
		ReplyDao replyDao=null;
