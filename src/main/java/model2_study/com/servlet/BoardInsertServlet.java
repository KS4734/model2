...
public class BoardInsertServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// 입력폼 (로그인 한 사람만 해당 폼에 이동가능, 로그인 하지않은 사람은 list로 이동)
		// 세션 객체도 파라미터에 포함된 상태
		String foardPage="./boardInsertForm.jsp";  // 저장 실패시 이동하는 페이지
		...
		if(loginUser_obj==null) {
			...
			session.setAttribute("msg", "게시글 등록은 로그인 후 가능합니다.");
		}else {
			req.getRequestDispatcher(foardPage).forward(req, resp);
		}
	}
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// 1. 저장 파라미터 처리, 2. 저장호출, 3. 경과에 따라 응답처리
