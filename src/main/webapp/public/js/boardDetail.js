...
function replyUpdateAct(e) {
	const form = e.target.form;  //입력요소에만 있는 선택자 form
...
async function boardPreferModify(e,prefer){  // prefer 1: 좋아요, 2: 싫어요
//promise 객체를 await로 호출할 수 있는 함수 : await를 호출하면 then((param)=>{}) param이 반환된다.
	...
	if(resp.status==200){
		...
		if(json.modify>0){
			let resp = await fetch("./boardPreferDetail.do?boardNo="+boardNo+"&prefer="+prefer);
			if(resp.status==200){
				let text = await resp.text();  // html
				preferContainer.innerHTML = text;
			}else{
				msg+=" 했지만 페이지를 불러오지 못했습니다. (새로고침 하세요!)";
			}
		}
	}else{
		msg="통신 실패!";
