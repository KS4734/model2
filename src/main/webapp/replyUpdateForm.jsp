...
action="./replyInsert.do" name="replyUpdate<%=reply.getReply_no()%>" method="post">
	...
	<input type="hidden" name="imgPath" value="<%=reply.getImg_path()%>">		
		<p>
			...
			<p><button type="reset">초기화</button>
			<button type="button" onclick="replyUpdateAct(event)">수정</button>
			<button type="button" onclick="replyDeleteAct(event)">삭제</button>
			</p>
</form>
