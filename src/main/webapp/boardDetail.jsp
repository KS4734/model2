...
#preferContainer button.prefer.active {
	box-shadow: inset -4px -2px 1px 1px rgb(0 0 0 / 80%);
}
#likesBtn {
...
Boolean prefer = null;  //true likes.active, false bads.active
...
%>
			<p id="preferContainer">
				...
				onclick="boardPreferModify(event,1)">좋아요<strong><%=board.getLikes()%></strong></button>
				
				<button id="badsBtn" value="<%=board.getBoard_no()%>" class="prefer
				<%=(prefer!=null&&!prefer)?"active":""%>" 
				onclick="boardPreferModify(event,0)">싫어요<strong><%=board.getBads()%></strong></button>
			</p>
			<p>
		...
		<div id="imgContainer">
			<%if(board.getBoardImgList() != null){
				...
				%>
						<img alt="게시글 이미지" src="./public/img/<%=boardImg.getImg_path() %>">
					<%}
					}%>
		</div>
		<div>
